...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in MilliBee.
...
10. [Creating a New Provider in MilliBee](#creating-a-new-provider-in-millibee)
...
const antigravityPlugin = {
  ...
  register(api: MilliBeePluginApi) {
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<MilliBeeConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- MilliBee codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.millibee/auth.json`:
...
## Creating a New Provider in MilliBee

MilliBee providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/millibee/cmd_auth.go`:
...
millibee auth login --provider your-provider

# List models (for Antigravity)
millibee auth models

# Start the gateway
millibee gateway

# Run an agent with a specific model
millibee agent -m "Hello" --model your-model
```
...
export MILLIBEE_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export MILLIBEE_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `millibee auth login --provider antigravity`
...
- Check auth profile storage: `~/.millibee/auth.json`
- Re-run `millibee auth login --provider antigravity`
