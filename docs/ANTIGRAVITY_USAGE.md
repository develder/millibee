# Using Antigravity Provider in MilliBee

This guide explains how to set up and use the **Antigravity** (Google Cloud Code Assist) provider in MilliBee.
...
```bash
millibee auth login --provider antigravity
```
...
6.  **Paste it back into the terminal** where MilliBee is waiting.

MilliBee will extract the authorization code and complete the process automatically.
...
### Switch Models
You can change the default model in `~/.millibee/config.json` or override it via the CLI:

```bash
# Override for a single command
millibee agent -m "Hello" --model claude-opus-4-6-thinking
```
...
1.  **Environment Variables**:
    *   `MILLIBEE_AGENTS_DEFAULTS_MODEL=gemini-flash`
2.  **Authentication persistence**: 
    If you've logged in locally, you can copy your credentials to the server:
    ```bash
    scp ~/.millibee/auth.json user@your-server:~/.millibee/
    ```
...
*   **429 Rate Limit**: Antigravity has strict quotas. MilliBee will display the "reset time" in the error message if you hit a limit.
*   **404 Not Found**: Ensure you are using a model ID from the `millibee auth models` list. Use the short ID (e.g., `gemini-3-flash`) not the full path.
