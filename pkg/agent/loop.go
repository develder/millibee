	...
	for al.running.Load() {
		...
		default:
			...
			if al.channelManager != nil {
				...
				fmt.Printf("[DEBUG] streaming: channel=%s chatID=%s onChunk=%v\n", msg.Channel, msg.ChatID, onChunk != nil) // TODO: remove
			}
...
func (al *AgentLoop) runAgentLoop(ctx context.Context, agent *AgentInstance, opts processOptions) (string, error) {
	...
	// 3. Save user message to session (skip for stateless requests like heartbeat)
	if !opts.NoHistory {
	...
	// 6. Save final assistant message to session (skip for stateless requests like heartbeat)
	if !opts.NoHistory {
		agent.Sessions.AddMessage(opts.SessionKey, "assistant", finalContent)
		agent.Sessions.Save(opts.SessionKey)
	}

	// 7. Optional: send response via bus (before summarization to avoid race)
	...
	// 9. Optional: background summarization (after response is sent)
	...
	if tokenEstimate > threshold {
		...
		if _, loading := al.summarizing.LoadOrStore(summarizeKey, true); !loading {
			go func() {
				...
				logger.InfoCF("agent", "Background summarization completed", map[string]any{
					"session_key": sessionKey,
				})
			}()
		}
...
func (al *AgentLoop) forceCompression(agent *AgentInstance, sessionKey string) {
	...
	// Find a clean cut point near the midpoint.
	// Avoid splitting inside a tool-call sequence (assistant → tool results).
	mid := len(conversation) / 2
	for mid < len(conversation)-1 && conversation[mid].Role == "tool" {
		mid++
	}
